...
const resolveTicket = async({params, render}) => {
  await executeQuery('UPDATE tickets SET resolved_on = NOW() WHERE id = $1', params.id);
  await listTickets({render: render});
}

const deleteTicket = async({params, render}) => {
  await executeQuery('DELETE FROM tickets WHERE id = $1', params.id);
  await listTickets({render: render});
}
